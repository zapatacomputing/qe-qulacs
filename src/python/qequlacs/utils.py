...
def append_gate(gate, qulacs_circuit):
    ...
    # Two qubit gate
        ...
        if gate.name == 'CNOT':
            ...
            qulacs_circuit.add_multi_Pauli_rotation_gate([index_1, index_2], [1,1], -gate.params[0]*2/np.pi)
        elif gate.name == 'YY':
            qulacs_circuit.add_multi_Pauli_rotation_gate([index_1, index_2], [2,2], -gate.params[0]*2/np.pi)
        elif gate.name == 'ZZ':
            qulacs_circuit.add_multi_Pauli_rotation_gate([index_1, index_2], [3,3], -gate.params[0]*2/np.pi)
        elif gate.name == 'CPHASE':
            mat = np.diag([1., np.exp(1.j*np.pi*gate.params[0])])
